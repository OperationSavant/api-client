...
// import { MessageEnvelope } from '@/shared/types/webview-messages';
...
// 		try {
// 			switch (command) {
// 				case CLIENT_COMMANDS.START_OAUTH2_AUTHORIZATION:
// 					// Browser-based flows: authorization_code, PKCE, implicit
// 					return await this.handleAuthorizationFlow(payload);

// 				case CLIENT_COMMANDS.EXCHANGE_OAUTH2_CODE:
// 					// Exchange authorization code for token (after browser callback)
// 					return await this.handleCodeExchange(payload);
// 				case CLIENT_COMMANDS.GENERATE_OAUTH2_TOKEN:
// 					// Direct token flows: password, client_credentials, refresh, device
// 					return await this.handleDirectTokenFlow(payload);

// 				case CLIENT_COMMANDS.REQUEST_DEVICE_CODE:
// 					// Device code flow - step 1: get device code
// 					return await this.handleDeviceCodeRequest(payload);
// 				default:
// 					throw new Error(`Unknown OAuth2 command: ${command}`);
// 			}
// 		} catch (error) {
// 			this.sendErrorResponse(error instanceof Error ? error.message : 'OAuth2 flow failed');
// 		}
// 	}
...
// 	 */
// 	private async handleAuthorizationFlow(config: OAuth2Auth): Promise<void> {
// 		const { grantType } = config;

// 		switch (grantType) {
// 			case 'authorization_code':
// 				return this.startAuthorizationCode(config);

// 			case 'authorization_code_pkce':
// 				return this.startAuthorizationCodePKCE(config);

// 			case 'implicit':
// 				return this.startImplicitFlow(config);
...
// 	 */
// 	private startAuthorizationCode(config: OAuth2Auth): void {
...
// 	 */
// 	private startAuthorizationCodePKCE(config: OAuth2Auth): void {
...
// 	 */
// 	private startImplicitFlow(config: OAuth2Auth): void {
...
// 	 */
// 	private async handleCodeExchange(config: any): Promise<void> {
...
// 		this.sendTokenResponse(response);
// 	}
...
// 	 */
// 	private async handleDirectTokenFlow(config: any): Promise<void> {
// 		const { grantType } = config;

// 		switch (grantType) {
// 			case 'password':
// 				return this.handlePasswordFlow(config);

// 			case 'client_credentials':
// 				return this.handleClientCredentialsFlow(config);

// 			case 'refresh_token':
// 				return this.handleRefreshTokenFlow(config);

// 			case 'urn:ietf:params:oauth:grant-type:device_code':
// 				return this.handleDeviceCodeTokenFlow(config);

// 			default:
// 				// Support custom grant types
// 				return this.handleCustomGrantType(config);
// 		}
// 	}
...
// 	 */
// 	private async handlePasswordFlow(config: any): Promise<void> {
// 		const body = new URLSearchParams();
...
// 	 */
// 	private async handleClientCredentialsFlow(config: any): Promise<void> {
// 		const body = new URLSearchParams();
...
// 	 */
// 	private async handleRefreshTokenFlow(config: any): Promise<void> {
// 		const body = new URLSearchParams();
...
// 	 */
// 	private async handleDeviceCodeTokenFlow(config: any): Promise<void> {
// 		const body = new URLSearchParams();
...
// 	 */
// 	private async handleCustomGrantType(config: any): Promise<void> {
// 		const body = new URLSearchParams();
...
// 	 */
// 	private async handleDeviceCodeRequest(config: any): Promise<void> {
// 		try {
// 			const body = new URLSearchParams();
// 			body.append('client_id', config.clientId);
...
// 				// 	'Copy URL'
// 				// );
// 				//TODO: Use BroadcastrHub to support multiple panels and don't use native browser APIs
...
// 				// }
// 			} else {
// 				this.sendErrorResponse(response.data.error || 'Failed to get device code', response.data.error_description);
// 			}
// 		} catch (error) {
// 			this.sendErrorResponse(error instanceof Error ? error.message : 'Device code request failed');
// 		}
// 	}
...
// 	 */
// 	private sendTokenResponse(response: any): void {
// 		if (response.status >= 200 && response.status < 300) {
// 			const data = response.data;

// 			if (!data.access_token) {
// 				this.sendErrorResponse('No access token in response', 'Token endpoint returned success but no access_token field');
// 				return;
// 			}
...
// 			this.sendErrorResponse(errorData.error || `HTTP ${response.status}`, errorData.error_description || response.statusText);
// 		}
// 	}
...
// 	 */
// 	private sendErrorResponse(error: string, description?: string): void {
// 		//TODO: Use BroadcastrHub to support multiple panels
// 		// panel.webview.postMessage({
// 		// 	command: 'oauth2TokenResponse',
// 		// 	success: false,
