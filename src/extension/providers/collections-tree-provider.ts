...
export class CollectionsTreeProvider implements vscode.TreeDataProvider<CollectionTreeItem> {
	private _onDidChangeTreeData: vscode.EventEmitter<CollectionTreeItem | undefined | null | void> = new vscode.EventEmitter<
	...
	private getCollections(): CollectionTreeItem[] {
		...
		return collections.map(
			collection =>
				new CollectionTreeItem(collection.name, vscode.TreeItemCollapsibleState.Collapsed, {
	...
	private getRequestsInCollection(collection: Collection): CollectionTreeItem[] {
		...
		return requests.map(
			request =>
				new CollectionTreeItem(`${request.method} ${request.name}`, vscode.TreeItemCollapsibleState.None, {
