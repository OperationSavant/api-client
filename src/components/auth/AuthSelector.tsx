...
const AuthSelector: React.FC<AuthSelectorProps> = ({ auth, onChange }) => {
	...
	const renderAuthComponent = () => {
		switch (auth.type) {
			...
			case 'oauth2':
				return auth.oauth2 ? <OAuth2AuthComponent auth={auth.oauth2} onChange={handleAuthConfigChange('oauth2')} /> : null;

			case 'aws':
				return auth.aws ? <AwsAuth auth={auth.aws} onChange={handleAuthConfigChange('aws')} /> : null;

			default:
				return <div className='text-sm text-muted-foreground'>This request does not use any authorization.</div>;
		}
	};
	...
	return (
		<div className='space-y-4'>
			...
			{auth.type !== 'none' && <div className='border-t pt-4'>{renderAuthComponent()}</div>}
		</div>
	);
};
