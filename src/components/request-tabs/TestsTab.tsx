...
const TestsTab: React.FC<TestsTabProps> = ({ testSuites, onTestSuitesChange, onRunTests, testExecutions = [], isRunning = false }) => {
	...
	const updateTestSuite = (suiteId: string, updates: Partial<TestSuite>) => {
		const updated = testSuites.map(suite => (suite.id === suiteId ? { ...suite, ...updates } : suite));
	...
	const updateAssertion = (suiteId: string, assertionId: string, updates: Partial<TestAssertion>) => {
		...
		const updatedAssertions = suite.assertions.map(assertion => (assertion.id === assertionId ? { ...assertion, ...updates } : assertion));
		...
		<div className='space-y-4'>
			<div className='flex items-center justify-between'>
				...
				<div className='flex gap-2'>
					<Button onClick={() => addTestSuite()} size='sm' variant='outline' className='flex items-center gap-2'>
			...
			<Tabs value={activeTab} onValueChange={value => setActiveTab(value as 'setup' | 'results')}>
								...
								<Card>
									<CardHeader>
										<div className='flex items-center justify-between'>
											...
											<div className='flex items-center gap-2'>
												<Checkbox checked={currentSuite.enabled} onCheckedChange={checked => updateTestSuite(selectedSuite, { enabled: !!checked })} />
				...
				<TabsContent value='results' className='space-y-4'>
					{testExecutions.length === 0 ? (
						...
						<div className='space-y-4'>{latestExecution && <TestResults execution={latestExecution} />}</div>
					)}
				</TabsContent>
			</Tabs>
		</div>
	...
	return (
		<Card className='bg-muted/30'>
			<CardContent className='p-4 space-y-3'>
				<div className='flex items-center gap-2'>
					<Checkbox checked={assertion.enabled} onCheckedChange={checked => onChange({ enabled: !!checked })} />
					...
					/>
					<Button onClick={onDelete} size='sm' variant='ghost' className='text-destructive hover:text-destructive hover:bg-destructive/10'>
						<Trash2 className='w-4 h-4' />
					</Button>
				</div>
				...
				<div className='grid grid-cols-12 gap-2 items-end'>
					{requiresField && (
						<div className='col-span-4 space-y-1'>
							<Label className='text-xs text-muted-foreground'>{config?.fieldDescription || 'Field'}</Label>
							<Input
		...
		<Card>
			<CardHeader>
				<div className='flex items-center justify-between'>
					...
					<Badge variant={execution.status === 'passed' ? 'default' : 'destructive'}>{execution.status.toUpperCase()}</Badge>
				</div>
			</CardHeader>
