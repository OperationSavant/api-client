      ...
      <div className="p-4 space-y-4 flex-1 overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={(value) => handleTabChange(value as BodyType)}>
          ...
          <TabsContent value="form-data" className="mt-4">
            <FormDataBody
              formData={bodyConfig.formData}
              onChange={(formData) => 
                handleBodyConfigChange({ ...bodyConfig, formData })
              }
            />
          </TabsContent>
          ...
          <TabsContent value="x-www-form-urlencoded" className="mt-4">
            <UrlEncodedBody
              urlEncoded={bodyConfig.urlEncoded}
              onChange={(urlEncoded) => 
                handleBodyConfigChange({ ...bodyConfig, urlEncoded })
              }
            />
          </TabsContent>
          ...
          <TabsContent value="raw" className="mt-4">
            <RawBody
              rawConfig={bodyConfig.raw}
              onChange={(raw) => 
                handleBodyConfigChange({ ...bodyConfig, raw })
              }
            />
          </TabsContent>
          ...
          <TabsContent value="binary" className="mt-4">
            <BinaryBody
              binaryConfig={bodyConfig.binary}
              onChange={(binary) => 
                handleBodyConfigChange({ ...bodyConfig, binary })
              }
            />
          </TabsContent>
          ...
          <TabsContent value="graphql" className="mt-4">
            <GraphQLBody
              graphqlConfig={bodyConfig.graphql}
              onChange={(graphql) => 
                handleBodyConfigChange({ ...bodyConfig, graphql })
              }
            />
          </TabsContent>
        </Tabs>
