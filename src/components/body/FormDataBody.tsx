...
const FormDataBody: React.FC<FormDataBodyProps> = ({ formData, onChange }) => {
	...
	const updateField = (index: number, updates: Partial<FormDataField>) => {
		const updated = formData.map((field, i) => (i === index ? { ...field, ...updates } : field));
		onChange(updated);
	};
				...
				<div className='space-y-3'>
					{formData.map((field, index) => (
						<div key={index} className='flex items-center gap-2 p-3 border rounded-lg'>
										...
										<div className='flex-1 flex items-center gap-2'>
											...
											<Label htmlFor={`file-${index}`} className='flex-1 cursor-pointer'>
												<div className='flex items-center gap-2 px-3 py-2 border rounded-md hover:bg-muted/50'>
													...
													<span className='text-sm truncate'>{field.fileName || 'Choose file...'}</span>
												</div>
											</Label>
										</div>
									)}
							...
							<Button onClick={() => removeField(index)} size='sm' variant='ghost' className='text-destructive hover:text-destructive hover:bg-destructive/10'>
