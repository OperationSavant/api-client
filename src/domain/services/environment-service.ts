	...
	public importData(data: any): void {
		if (data) {
			if (data.scopes && Array.isArray(data.scopes)) {
				const revivedScopes = data.scopes.map(([id, scope]: [string, EnvironmentScope]) => {
					const revivedScope: EnvironmentScope = {
						...scope,
						createdAt: new Date(scope.createdAt),
						updatedAt: new Date(scope.updatedAt),
						variables: scope.variables.map((variable: EnvironmentVariable) => ({
							...variable,
							createdAt: new Date(variable.createdAt),
							updatedAt: new Date(variable.updatedAt),
						})),
					};
					return [id, revivedScope];
				});
				this.scopes = new Map(revivedScopes);
			} else {
				this.scopes = new Map();
			}
