  ...
  private evaluateAssertion(assertion: TestAssertion, actualValue: any): boolean {
    ...
    switch (operator) {
      ...
      case 'in-range':
        ...
        try {
          const [min, max] = String(expectedValue).split(',').map(v => Number(v.trim()));
 ...
 */
export const createResponseTestData = async (
  response: Response,
  responseTime: number
): Promise<ResponseTestData> => {
